/**
* @file vector_4d_operator.hpp
* @brief vector4クラスの演算子
* @author 石山 悠
* @date 2018/12/13
*/
#ifndef SAKI_VECTOR_DETAILS_4D_VECTOR_4D_OPERATOR_HPP
#define SAKI_VECTOR_DETAILS_4D_VECTOR_4D_OPERATOR_HPP
#include <utility>
...
{
	return vector4<decltype(std::declval<T1>() + std::declval<T2>())>{f(v1.x, v2.x), f(v1.y, v2.y), f(v1.z, v2.z), f(v1.w, v2.w)};
}
/**
		* @brief ベクトルとスカラの演算
		*/
...
{
	return vector4<decltype(std::declval<T1>() * std::declval<T2>())>{f(v.x, scalar), f(v.y, scalar), f(v.z, scalar), f(v.w, scalar)};
}
} // namespace details
/**
	* @brief +演算子
...
/**
	...
	*/
template <typename T1, typename T2>
[[deprecated("type_mismatch")]] constexpr bool operator==(const vector4<T1> &v1, const vector4<T2> &v2)
{
	return v1.x == v2.x && v1.y == v2.y && v1.z == v2.z && v1.w == v2.w;
}
/**
...
#endif //SAKI_VECTOR_DETAILS_4D_VECTOR_4D_OPERATOR_HPP