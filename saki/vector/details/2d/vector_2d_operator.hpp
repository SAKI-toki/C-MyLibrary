/**
* @file vector_2d_operator.hpp
* @brief vector2クラスの演算子
* @author 石山 悠
* @date 2018/12/13
*/
#ifndef SAKI_VECTOR_DETAILS_2D_VECTOR_2D_OPERATOR_HPP
#define SAKI_VECTOR_DETAILS_2D_VECTOR_2D_OPERATOR_HPP
#include <utility>
...
{
	return vector2<decltype(std::declval<T1>() + std::declval<T2>())>{f(v1.x, v2.x), f(v1.y, v2.y)};
}
/**
* @brief ベクトルとスカラの演算
*/
...
{
	return vector2<decltype(std::declval<T1>() * std::declval<T2>())>{f(v.x, scalar), f(v.y, scalar)};
}
} // namespace details
/**
* @brief +演算子
...
*/
template <typename T1, typename T2>
[[deprecated("type_mismatch")]] constexpr bool operator==(const vector2<T1> &v1, const vector2<T2> &v2)
{
	return v1.x == v2.x && v1.y == v2.y;
}
/**
...
SAKI_NAMESPACE_END
#endif //SAKI_VECTOR_DETAILS_2D_VECTOR_2D_OPERATOR_HPP