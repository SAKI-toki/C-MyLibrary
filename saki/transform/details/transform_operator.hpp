/**
* @file transform_operator.hpp
* @brief transformクラスの演算子
* @author 石山 悠
* @date 2018/12/16
*/
#ifndef SAKI_TRANSFORM_DETAILS_TRANSFORM_OPERATOR_HPP
#define SAKI_TRANSFORM_DETAILS_TRANSFORM_OPERATOR_HPP
#include <utility>
...
{
	return saki::transform<decltype(std::declval<T1>() + std::declval<T2>())>{f(v1.get_pos(), v2.get_pos()), f(v1.get_rot(), v2.get_rot()), f(v1.get_scale(), v2.get_scale())};
}
/**
...
{
	return saki::transform<decltype(std::declval<T1>() * std::declval<T2>())>{f(v.get_pos(), scalar), f(v.get_rot(), scalar), f(v.get_scale(), scalar)};
}
} // namespace details
/**
* @brief +演算子
...
*/
template <typename T1, typename T2>
[[deprecated("type_mismatch")]] constexpr bool operator==(const saki::transform<T1> &v1, const saki::transform<T2> &v2)
{
	return v1.get_pos() == v2.get_pos() && v1.get_rot() == v2.get_rot() && v1.get_scale() == v2.get_scale();
}
/**
...
SAKI_NAMESPACE_END
#endif //SAKI_TRANSFORM_DETAILS_TRANSFORM_OPERATOR_HPP