...
namespace saki
{
	...
	private:
		//ルートノード
		std::shared_ptr<saki::node<T>> root;
	public:
		/**
		* @brief 引数ありコンストラクタ
		* @param name ルートノード名
		...
		*/
		...
		{
			root = std::make_shared<saki::node<T>>(root, name, t);
		}
		/**
		* @brief ノードの追加
		...
		*/
		std::shared_ptr<saki::node<T>> get_node(const std::string& node_path = "")const
		{
			...
			if (*str_itr == ".")++str_itr;
			//カレントからスタートする
			std::shared_ptr<saki::node<T>> node = root;
			//ノードをめぐる
		...
		* @return root
		*/
		std::shared_ptr<saki::node<T>> get_root()const
		{
			return root;
		}
		/**
		...
		*/
		...
		{
			this->root = std::make_shared<saki::node<T>>(this->root, other.root->name, other.root->get_value());
